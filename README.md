...
프로필 화면에서 비밀번호 변경 기능을 구현하였다.
현재 비밀번호와 새 비밀번호를 입력받아 형식 검사와 일치 여부를 확인한 후, 암호화하여 서버로 전송하도록 구성하였다.
서버에서는 저장된 비밀번호와 비교하여 일치할 경우 새 비밀번호로 업데이트하며,
변경 성공 시 Toast 알림을 띄운 후 자동으로 로그아웃되어 로그인 페이지로 이동하도록 하였다.
## 13주차수업내용
...
실습5 : 브라우저 기본 알림창 추가
실습6 : profile.html - 비밀번호 변경 폼 추가, Toast 알림창 추가
실습7 : profile.js - 입력값 검사(빈값, 형식, 일치 여부), 비밀번호 암호화 후 서버 전송, 변경 성공 시 알림 후 자동 로그아웃
실습8 : AuthResource.java - 비밀번호 변경 기능 추가, 로그아웃 시 로그인 페이지로 이동 기능 추가
과제1 : 기존 미구현된 alert 들을 Toast로 구현
<divalign="center">
...
<img src="screenshot/13주차 (6).png" width="45%" alt="실습 화면">
<img src="screenshot/13주차 (7).png" width="45%" alt="실습 화면">
</div>
<br>
...
비밀번호를 바꿀 때 그냥 전송하는 게 아니라 암호화해서 보내야 한다는 걸 알게 됐다.
또 변경이 완료되면 자동으로 로그아웃되고 로그인 페이지로 넘어가는 기능을 만들면서 실제 서비스처럼 동작하는 걸 보니 재미있었다.


## 최종과제 마무리 완로
한 학기 동안 프론트엔드 UI 설계부터 JavaScript 동적 처리, Java 백엔드 개발, 데이터베이스 연동까지 웹 개발의 전체 흐름을 직접 경험할 수 있어서 매우 유익한 수업이었다.
이번 학기, Quarkus를 활용한 자바 웹 프로그래밍 수업을 들으면서 개발자가 갖춰야 할 기본적인 흐름을 확실히 잡을 수 있었습니다.
처음 2주차에 개발 환경을 세팅할 때는 복잡한 설정들 때문에 막막하기도 했지만,
마지막 13주차에 회원 정보 수정과 보안 기능까지 구현해내며 웹 서비스의 시작과 끝을 경험해본 건 정말 값진 시간이었습니다.
학기 초에는 그냥 HTML과 CSS로 화면을 꾸미는 것만으로도 신기했습니다.
하지만 9주차부터 MySQL 데이터베이스를 연동하고, Java로 백엔드 로직을 짜기 시작하면서 '아, 프론트엔드와 백엔드가 이렇게 데이터를 주고받는구나'라는 걸 몸소 체감했습니다.
특히 로그인부터 회원가입, 프로필 수정까지 이어지는 과정을 직접 엔드포인트로 설계해보니, 실제 서비스가 어떤 원리로 돌아가는지 전체적인 그림을 그릴 수 있게 되었습니다.
Bootstrap을 사용하면서 UI 구성이 훨씬 수월해졌는데, 처음에는 단순히 갖다 쓰기만 하던 컴포넌트들을 나중에는 모달창, Toast 알림 등 사용자 경험(UX)을 고려해 배치하고 수정하는 단계까지 성장했습니다.
개발자 도구로 코드를 뜯어보고 문제점을 찾아가는 과정은 처음엔 어려웠지만, 반복하다 보니 이제는 코드 구조를 읽고 수정하는 게 꽤 익숙해졌습니다.
가장 기억에 남는 건 회원가입과 비밀번호 변경 구현입니다.
단순히 데이터를 저장하는 게 아니라, SHA-256 해시 암호화를 적용해보고, 검색어가 공백일 때의 예외 처리나 로그인 실패 시 에러 메시지 띄우기 같은 사소한 부분들이 서비스 완성도를 얼마나 크게 좌우하는지 깨달았습니다.
이런 디테일한 부분들을 스스로 고민해서 구현했을 때의 성취감이 정말 컸습니다.
이번 학기 실습을 통해 웹 개발에 대한 자신감이 많이 생겼습니다.
단순히 코드를 따라치는 수준에서 벗어나, 왜 이 코드가 필요한지 고민하고 더 나은 사용자 경험을 위해 설계해보는 즐거움을 알게 된 것이 가장 큰 수확인 것 같습니다.
이번 프로젝트에서 배운 기초를 바탕으로, 앞으로는 제가 직접 아이디어를 내서 구현해보는 더 발전된 개인 프로젝트에 도전해보고 싶습니다.